...
class DNAProvider(str, Enum):
    ...
    def parse_overlap_myheritage(data):
        mappings = {k: myheritage_mappings[k] for k in ('chromosome', 'start', 'end', 'centimorgans', 'name', 'matchname', 'snps')}
        mappings['matchname'] = "Match Name"  # Idiotic hack because MyHeritage can't decide on casing
        f = StringIO(data)
        reader = csv.DictReader(f)
    ...
    def parse_matchfile(provider, datafile):
        ...
        if zipfile.is_zipfile(datafile):
            filename = ".".join(os.path.basename(datafile).split('.')[0:-1]) + ".csv"
            print(filename)
