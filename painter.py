...
def dircolor(v: float):
    paternal_a = (0.0, 1.0, 1.0)  # Father's father line,
    paternal_b = (.4, 1.0, 1.0)  # Father's mothers line
    maternal_a = (0.6, 1.0, 1.0)  # Mothers' father line
    maternal_b = (1.0, 1.0, 1.0)  # Mother's mother line
...
class Painter(object):
    ...
    def add_match(self, match):
        ...
        for segment in match['segments']:
            print(segment)
            mark = self.create_mark(segment['chromosome'], int(segment['start']), int(segment['end']))
            svggroup.add(mark) 
    ...
    def draw(self):
        ...
        for group_id, group in self.matches.items():
            ...
            svggroup = self.dwg.g(stroke="none", opacity="0.8", fill=color_string,
                                  id=group_id)
...
def paint(tester, matches, gedfile):
    ...
    painting = Painter(tester['name'] + ".svg")
...
def main():
    matches = Matches(filename = os.path.join(SETTINGS.get('datafolder'), "gensplorer.json"))
